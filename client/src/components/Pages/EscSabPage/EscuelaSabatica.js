...
import styles from './EscuelaSabatica.module.css';

const API_BASE_URL = process.env.REACT_APP_API_URL || 'http://localhost:3001';
const MINISTRY_ID_ES = 2; 

function EscuelaSabatica() {
    ...
    //Obtiene los archivos asociados específicamente al Ministerio de Escuela Sabática desde la API, usando su ID predefinido.
    const fetchMinistryFiles = useCallback(async () => {
        setLoadingFiles(true); 
        setFilesError(null);   
        try {
            const token = localStorage.getItem('authToken');
            const response = await axios.get(`${API_BASE_URL}/archivos/ministry/${MINISTRY_ID_ES}`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            ...
            setFilesError("No se pudieron cargar los archivos de Escuela Sabática. Inténtalo de nuevo más tarde.");
    ...
    return (
        <div className={styles.escuelaSabaticaContainer}>
            {/* Sección del Encabezado (HEADER) */}
            <header className={styles.escuelaSabaticaHeader}>
                <div className={styles.headerContent}>
                    <h2 className={styles.headerSubtitle}>MINISTERIO</h2>
                    <h1 className={styles.headerTitle}>ESCUELA <span>Sabatica</span></h1>
                </div>
                ...
                <nav className={styles.headerNav}>
                    <ul>
                        ...
                        <li><a href="#recursos-maestros" className={styles.navLink}>RECURSOS MAESTROS</a></li>
                        <li><a href="#archivos-ministerio" className={styles.navLink}>ARCHIVOS DEL MINISTERIO</a></li>
                    </ul>
                </nav>
            </header>
            ...
            <div className={styles.resourceContentArea}>
                ...
                <section className={styles.highlightSection} id='destaque'>
                    <h2 className={styles.sectionHeading}>Destaque</h2>
                    <div className={styles.highlightCard}>
                        <img src="/Images/AniversarioES.jpg" alt="Auxiliares de Escuela Sabática Trimestre Actual" className={styles.highlightImage} />
                        <div className={styles.highlightDetails}>
                            <h3 className={styles.highlightTitle}>Aniversário 173 años - Escuela Sabática</h3>
                            <p className={styles.highlightDescription}>En 2025, celebramos con alegría los 173 años de la Escuela Sabática. Es un momento especial para reconocer el papel vital de este movimiento en el estudio de la Biblia y en nuestra jornada espiritual. ¡Descarga los materiales y celebra esta fecha significativa en tu iglesia! La iglesia Adventista conmemora el aniversario de la Escuela Sabática, siempre el segundo sábado de octubre.</p>
                            <a href="#" className={styles.highlightLink}>VER MÁS <span className={styles.arrowIcon}>&gt;</span></a>
                        </div>
                    </div>
                </section>

                {/* Sección de Recursos para el Maestro */}
                <section className={styles.categorySection} id='recursos-maestros'>
                    <h2 className={styles.sectionHeading}>Recursos para el Maestro</h2>
                    <div className={styles.resourceGrid}>
                        <div className={styles.resourceItem}>
                            <img src="/Images/CursoMaestro.jpg" alt="Guía Adicional del Maestro" className={styles.resourceThumbnail} />
                            <div className={styles.resourceInfo}>
                                <h4 className={styles.resourceTitle}>Guía del Maestro - Recursos Adicionales</h4>
                                <a href="https://downloads.adventistas.org/es/escuela-sabatica/materiales-de-divulgacion/clase-de-maestro/" target="_blank" rel='noopener noreferrer' className={styles.resourceLink}>VER MÁS <span className={styles.arrowIcon}>&gt;</span></a>
                            </div>
                        </div>
                        <div className={styles.resourceItem}>
                            <img src="/Images/CursoLiderazgo.jpg" alt="Seminario de la Escuela Sabática" className={styles.resourceThumbnail} />
                            <div className={styles.resourceInfo}>
                                <h4 className={styles.resourceTitle}>Seminario de Liderazgo de la Escuela Sabática</h4>
                                <a href="https://downloads.adventistas.org/es/ministerio-de-los-adolescentes/manuales-y-guias/curso-de-liderazgo-del-mca-nivel-8/" target="_blank" rel='noopener noreferrer' className={styles.resourceLink}>VER MÁS <span className={styles.arrowIcon}>&gt;</span></a>
                            </div>
                        </div>
                    </div>
                </section>
                                ...
                                return (
                                    <li key={file.ArchivoID} className={styles.fileItem}>
                                        ...
                                        <div className={styles.fileActions}>
                                            ...
                                            {/* Botón de Descargar */}
                                            {/* Nota: `download` atributo solo sugiere un nombre, la descarga real la maneja el navegador */}
                                            <a
                                                href={url}
                                                download={file.NombreArchivo} // Sugiere el nombre original para la descarga
            ...
            <UploadModal
                ...
                preselectedMinistryId={MINISTRY_ID_ES} // Preselecciona el ministerio para este componente
                showSwalAlert={showSwalAlert}
                uploadUrl={`${API_BASE_URL}/archivos/uploads`} // Se asume que esta URL es para la subida física
            />

            {/* Reproductor de Audio (visible solo si hay una URL de audio) */}
            {currentAudioUrl && (
                <AudioPlayer src={currentAudioUrl} onClose={handleCloseAudioPlayer} />
            )}

            {/* Reproductor de Video (visible solo si hay una URL de video) */}
            {currentVideoUrl && (
...
export default EscuelaSabatica;